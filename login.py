...
class LoginWindow:
    ...
    def login(self, user):  # 1)user 0: ∞ clearance  2)user 1: level 1 clearance 3) user 2: level 2 clearance
        ...
        if user == 0:
            ...
            create_label(text="Doctor", x=125, y=130, fg='#ffff00', bg='#000000')
        elif user == 2:
        ...
        if user == 0:
            ...
            forgot_password = Button(self.frame, text="Forgot password?", bd=0, bg='#000000', fg='#0000ff',
                                     ...
                                     command=lambda: self.forgotPassword())
            ...
            register = Button(self.frame, text="Create New Account", bd=0, bg='#000000', fg='#0000ff',
                              cursor='hand2', activebackground='#000000', activeforeground='#ff000f',
                              command=lambda: self.creatNewAccount())
            register.place(x=97, y=390, width=160)
            register.configure(font=("Times New Roman", 10, "bold"))

        # # back
        # click_btn = PhotoImage(file='img/back_key.png')
        # back = Button(self.root, image=click_btn, bd=3, relief=RIDGE, bg='#050542', fg='#f6f0f0', cursor='hand2',
        #               activebackground='#050542', activeforeground='#f6f0f0', command=lambda: Welcome())
        # back.place(x=645, y=310, width=160, height=30)
        # back.configure(font=("Times New Roman", 10, "bold"))
